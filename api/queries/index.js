...
(function() {
  exports.getArgument = "RETURN { argument: ( FOR a IN argument FILTER a._id == @argId RETURN a )[0], premises: ( FOR premiseStatement IN GRAPH_NEIGHBORS(@graphName, @argId, {edgeCollectionRestriction: 'premise'}) RETURN { statement: ( FOR s IN statement FILTER s._id == premiseStatement RETURN s )[0], subargument: ( FOR subargument IN GRAPH_NEIGHBORS(@graphName, premiseStatement, {edgeCollectionRestriction: 'conclusion'}) RETURN subargument ) } ), conclusion: ( FOR conclusionStatement IN GRAPH_NEIGHBORS(@graphName, @argId, {edgeCollectionRestriction: 'conclusion'}) RETURN { statement: ( FOR s IN statement FILTER s._id == conclusionStatement RETURN s )[0] } )[0], comments: ( FOR commentStatement IN GRAPH_NEIGHBORS(@graphName, @argId, {edgeCollectionRestriction: 'comment'}) RETURN { statement: ( FOR s IN statement FILTER s._id == commentStatement RETURN s )[0], subargument: ( FOR subargument IN GRAPH_NEIGHBORS(@graphName, commentStatement, {edgeCollectionRestriction: 'conclusion'}) RETURN subargument ) } ) }";
